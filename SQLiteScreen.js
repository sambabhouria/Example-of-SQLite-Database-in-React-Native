...
export default class SQLiteScreen extends React.Component {
  ...
  async CreateTable() {
    ...
    let userTable = await this.ExecuteQuery(
    ...
    let stateTable = await this.ExecuteQuery(
      'CREATE TABLE IF NOT EXISTS country (id INTEGER PRIMARY KEY NOT NULL, user_id INTEGER, country_name VARCHAR(16), is_deleted INTEGER)',
      [],
    );
